...
def detect_phase(ctx: AuctionContext) -> Category:
...
def collect_rules(registry: RuleRegistry, ctx: AuctionContext) -> list[Rule]:
    """Gather phase rules + overlay rules, sorted by priority descending."""
    phase = detect_phase(ctx)
    rules: list[Rule] = list(registry.rules_for(phase))
    for overlay in _OVERLAY_CATEGORIES:
        rules.extend(registry.rules_for(overlay))
...
class BidSelector:
    ...
    def _collect_rules(self, ctx: BiddingContext) -> list[Rule]:
        return collect_rules(self._registry, ctx)
